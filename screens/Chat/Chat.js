...
export default function Chat({route, navigation}) {
  ...
  let [chatMessaged] = useState([
    ...
    },
    {
      name: 'ShaMsiDDin',
      receiver: false,
      text: '111Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec fringilla quam eu faci lisis mollis.'
    },
  ...
  if (!fontsLoaded) {
    ...
    return (
      <TouchableWithoutFeedback
        onPress={!isKeyboardVisible ? () => Keyboard.dismiss : scrollArea.scrollToEnd({animated: true})}
        accessible={false}>
        <View style={styles.chat}>
          ...
          <View style={[styles.chatHeader]}>
            <View
              style={[styles.header, !route.params ? {justifyContent: 'center'} : {justifyContent: 'space-between'}]}>
              {route.params ?
              ...
              <TouchableOpacity style={[styles.button, !route.params ? {width: '94%'} : null]}
                                onPress={!route.params ? () => navigation.navigate('Avaliable') : null}>
          ...
          <Animated.View
            style={[styles.container, {maxHeight: isKeyboardVisible ? Dimensions.get('window').height / 100 * 38 : Dimensions.get('window').height / 100 * 67}]}>
            <ScrollView ref={ref => setScrollArea(ref)}
                        onContentSizeChange={() => scrollArea.scrollToEnd({animated: true})}>
              {chatMessaged.map((item, index) => (
                <View onStartShouldSetResponder={() => true} key={index} style={[{
                  display: 'flex',
                  justifyContent: 'space-between',
                  backgroundColor: '#F5F5F5'
                }, item.receiver ? {alignItems: 'flex-end'} : {alignItems: 'flex-start'}]}>
                  <View key={index} style={[{
                    width: '70%',
                    marginTop: '1%'
                  }, item.receiver ? {marginRight: '4%'} : {marginLeft: '4%'}]}>
                    <Text
                      style={[{fontFamily: 'Poppins_500Medium'}, item.receiver ? {alignSelf: 'flex-end'} : {alignItems: 'flex-start'}]}>{item.name}</Text>
                    <View style={[styles.message, item.receiver ? styles.receiver : styles.sender]}>
                      <Text
                        style={[item.receiver ? {color: '#112B66'} : {color: 'white'}, {fontFamily: 'Poppins_400Regular'}]}>{item.text}</Text>
                    </View>
                  </View>
                  <View
                    style={[styles.TriangleShapeCSS, item.receiver ? styles.TriangleShapeRightCSS : styles.TriangleShapeLeftCSS]}/>
                </View>
              ))}
            </ScrollView>
          </Animated.View>
